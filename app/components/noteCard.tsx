...
export default memo(function NoteCard({ note, layoutId, onEdit, onDelete, onActivate }: NoteCardProps) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      exit={{ opacity: 0, y: -20 }}
      className="p-4 w-full flex flex-col justify-between hover:bg-purple-50 dark:hover:bg-purple-900/10 rounded-xl cursor-pointer bg-white dark:bg-gray-900 shadow-lg border border-purple-100"
    >
      <div className="flex justify-between">
        <div className="flex-1" onClick={handleActivate}>
          <motion.h3
            layoutId={`title-${note._id}-${layoutId}`}
            className="font-medium text-black dark:text-purple-100 text-lg"
          >
            {note.title}
          </motion.h3>
          <motion.div
            layoutId={`description-${note._id}-${layoutId}`}
            className="text-black dark:text-purple-200 text-sm mt-1 line-clamp-3"
          >
            {processedContent}
          </motion.div>
          <motion.div 
          ...
          >
            {note.tags?.map((tag: string) => (
              <motion.span
                ...
                initial={{ scale: 0.8 }}
                animate={{ scale: 1 }}
                className="px-2 py-1 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-200 text-xs rounded-full"
              >
                {tag}
              </motion.span>
            ))}
